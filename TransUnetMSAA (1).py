...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if config.patches.get("grid") is not None:   # ResNet
            ...
            patch_size = (img_size[0] // 7 // grid_size[0], img_size[1] // 7 // grid_size[1], img_size[2] // 7 // grid_size[2])
            print('111',patch_size)
        ...
        self.patch_embeddings = Conv3d(in_channels=in_channels, out_channels=config.hidden_size, kernel_size=patch_size, stride=patch_size)
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis):
        ...
        self.embeddings = Embeddings(config, img_size=img_size)#总数，维度
...
class SpatialAttention3D(nn.Module):
    ...
    def forward(self, x):
        avg_out = torch.mean(x, dim=1, keepdim=True)  # B x 1 x D x H x W
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        x = torch.cat([avg_out, max_out], dim=1)
        return self.sigmoid(self.conv(x))


class MSFFA3D(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.fusion = nn.Conv3d(2*in_channels, out_channels, kernel_size=1, stride=1)
        self.ca = ChannelAttention3D(out_channels)
        self.sa = SpatialAttention3D()

    def forward(self, x1, x2):
        x = torch.cat([x1, x2], dim=1)  # channel: in_channels
        x = self.fusion(x)
        x = self.ca(x) * x
        x = self.sa(x) * x
        return x
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels=0, use_batchnorm=True):
        ...
        # Add GAM module to process skip connection
        # 使用MSFFA3D模块处理跳跃连接特征
        if skip_channels > 0:
            # 创建三种尺寸的MSFFA3D模块（适应不同通道数）
            self.msffa_64 = MSFFA3D(in_channels=64, out_channels=64)
            self.msffa_256 = MSFFA3D(in_channels=256, out_channels=256)
            self.msffa_512 = MSFFA3D(in_channels=512, out_channels=512)
        else:
            self.msffa_64 = self.msffa_256 = self.msffa_512 = None
    def forward(self, x, skip=None):
        """处理跳跃连接
        参数:
            x: 来自上一解码块的特征 [B, C, D, H, W]
            skip: 跳跃连接特征 [B, skip_channels, D, H, W] 或 None
        """
        x = self.up(x)
        ...
        if skip is not None:
            # 根据通道数选择不同的MSFFA模块
            if skip.size(1) == 64:
                skip = self.msffa_64(skip, skip)  # 自注意力增强
            elif skip.size(1) == 256:
                skip = self.msffa_256(skip, skip)  # 自注意力增强
            elif skip.size(1) == 512:
                skip = self.msffa_512(skip, skip)  # 自注意力增强
...
class SegmentationHead(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel_size, upsampling=1):
        conv3d = nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=kernel_size // 2)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=128, num_classes=21843, zero_head=False, vis=False):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if self.transformer.embeddings.hybrid:
                tensor = np2th(res_weight["conv_root/kernel"])
                print("Tensor shape:", tensor.shape)  # 打印张量形状
...
if __name__ == '__main__':
    ...
    net = VisionTransformer(config, img_size=128, num_classes=21843, zero_head=False, vis=False)
